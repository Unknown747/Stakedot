#!/usr/bin/env python3
"""
Live Test Terbatas — 100 Ronde (UANG ASLI)
Menjalankan logika production main.py (jalankan_strategy_vip) dengan
batas keras 100 ronde, lalu berhenti otomatis. Tidak ada menu, tidak
ada pertanyaan interaktif — cocok untuk sekali jalan di VPS/shell.
"""

import sys
from main import (
    API_KEY, gql, USER_QUERY, jalankan_strategy_vip,
    g, BOLD, GREEN, RED, YELLOW, print_banner,
)

MAKS_RONDE = int(sys.argv[1]) if len(sys.argv) > 1 else 100
...
print_banner()
print(g(YELLOW, f"⏳ Menghubungkan ke Stake.com untuk live test {MAKS_RONDE} ronde (UANG ASLI)..."))
...
jalankan_strategy_vip(user, vps_mode=True, maks_ronde=MAKS_RONDE)
